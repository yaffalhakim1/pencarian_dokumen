...
import EditTable from "../../components/table/edit/[id]";
...
export default function CrudTable() {
  ...
  const { data, error, mutate } = useSWR(
    `https://spda.17management.my.id/api/tables/data?page=${page}`,
    fetcher
  );
  if (error)
    return (
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      await axios.post(
        `https://spda.17management.my.id/api/tables/delete/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <div className="container px-6  pt-2 pb-6 h-full">
        <p className="text-2xl font-semibold mb-2">Dashboard Meja</p>
